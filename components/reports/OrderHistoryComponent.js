            ...
            <View>
                ...
                <View style={styles.container}>
                    <FlatList
                        data={item_data}
                        renderItem={({ item, key }) => (
                            <LinearGradient start={{x: 0, y: 0}} end={{x: 1, y: 0}} colors={['#0072ff', '#00c6ff', ]} style={styles.cardView} key={key}>
                                    ...
                                    <Text style={{color:'#FFF', marginTop: 5}}>{item.created_at}</Text>
                                    {
                                        ...
                                        (
                                            <Badge style={ item.status == 5 ? { backgroundColor: 'green'} : { backgroundColor: 'red'}}>
                                                <Text style={{color: 'white', fontWeight: 'bold'}}>{ item.status == 5 ? 'Executed' : 'Cancelled'}</Text>
                                            </Badge>
                                        )
                                    }
                                ...
                                <View style={{padding: 5, flexDirection: 'row', alignItems: 'flex-end', justifyContent:'space-between'}}>
                                    <Text style={{fontSize:14, fontWeight:'bold', color:'#FFF'}}>Security Code: {item.client_code}</Text>
                                    <Text style={{color:'#FFF'}}>{item.name}</Text>
                                </View>
                                <View style={{padding: 5, flexDirection: 'row', alignItems: 'flex-end', justifyContent:'space-between'}}>
                                    <Text style={{color:'#FFF'}}>Current Rate: {item.bank_name}</Text>
                                </View>
                                <View style={{padding: 5, flexDirection: 'row', alignItems: 'flex-end', justifyContent:'space-between'}}>
                                    <Text style={{color:'#FFF'}}>Number of Share: {item.account_no}</Text>
                                </View>
                                <View style={{padding: 5, flexDirection: 'row', alignItems: 'flex-end', justifyContent:'space-between'}}>
